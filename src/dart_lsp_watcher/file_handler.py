"""File system event handler for Dart file watching."""
...
def is_dart_file(path: str) -> bool:
    """Check if the path is a Dart file.

    Args:
        path: File path to check.

    Returns:
        True if the file has a Dart extension.
    """
    return any(path.lower().endswith(ext) for ext in CONSTANTS.DART_EXTENSIONS)


def scan_dart_files(directory: str) -> list[str]:
    ...
    Returns:
        List of absolute paths to Dart files.
    """
    dart_files: list[str] = []
...
class DartFileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: FileSystemEvent) -> None:
        ...
        if event.is_directory or not is_dart_file(str(event.src_path)):
            return
