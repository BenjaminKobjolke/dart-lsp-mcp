"""MCP Server for Dart LSP diagnostics."""
...
from dart_lsp_watcher.config.constants import CONSTANTS
from dart_lsp_watcher.file_handler import scan_dart_files
...
# Create logger
logger = logging.getLogger("dart-lsp-mcp")
        ...
        if project_path not in _lsp_clients:
            ...
            if not client.start():
                raise RuntimeError(
                    ...
                    "Please ensure Dart SDK is installed: https://dart.dev/get-dart"
                )
            ...
            # Index all Dart files
            dart_files = scan_dart_files(project_path)
            logger.info(f"Indexing {len(dart_files)} Dart files...")
...
def _format_diagnostics(
    ...
    """
    from dart_lsp_watcher.diagnostics import (
...
def go_to_definition(
    ...
    Args:
        project_path: Absolute path to Dart project root.
...
def _start_http_server() -> HTTPServer | None:
    ...
    """
    port = int(os.environ.get("DART_HTTP_PORT", "19851"))
