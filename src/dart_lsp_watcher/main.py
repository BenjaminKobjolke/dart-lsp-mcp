"""Main entry point for Dart LSP Watcher CLI."""
...
from dart_lsp_watcher.config.config_file import get_ignore_patterns, load_config_file
from dart_lsp_watcher.config.constants import COLORS, CONSTANTS
from dart_lsp_watcher.config.settings import Settings
from dart_lsp_watcher.diagnostics import DiagnosticsDisplay
from dart_lsp_watcher.file_handler import DartFileHandler, scan_dart_files
from dart_lsp_watcher.lsp_client import LspClient
from dart_lsp_watcher.utils import normalize_uri, path_to_uri, uri_to_path
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Watch a Dart project folder and display Dart LSP diagnostics."
    )
    parser.add_argument("folder_path", help="Path to the Dart project folder to watch")
    parser.add_argument(
        "--min-severity",
        "-s",
...
def main() -> None:
    ...
    print(f"{COLORS.CYAN}Starting Dart LSP Watcher...{COLORS.RESET}")
    ...
    if args.references:
        ...
        # Scan and open ALL Dart files so LSP has full project context
        dart_files = scan_dart_files(settings.workspace_path)
    ...
    if args.file:
        ...
        # Open only the target file for fast single-file diagnostics
        lsp_client.open_document(abs_file)
    ...
    # Scan and open existing Dart files
    print("Scanning for Dart files...")
    dart_files = scan_dart_files(settings.workspace_path)
    print(f"Found {len(dart_files)} Dart file(s)")

    for file_path in dart_files:
        lsp_client.open_document(file_path)

    # Set up file watcher
    event_handler = DartFileHandler(lsp_client, debounce_delay=CONSTANTS.DEBOUNCE_DELAY)
    observer = Observer()
